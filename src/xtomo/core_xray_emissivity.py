...
def _line_integral_element(M: int, bz: float, pnorm: float, nbins: int = 1001) -> float:
    """
...
def _build_line_integral_matrix(
    phi: np.ndarray,
    pnorm: np.ndarray,
    cos_m_vals,
    sin_m_vals,
...
def core_xray_emissivity(
    ...
    *,
    ...
    lmax: int = 15,
    svd_tol: float = 0.1,
    chord_mask: np.ndarray | None = None,
    ...
    chord_mask      : optional boolean mask over all 76 chords (array1 then array3)
                      False entries are removed from both inversion equations and
                      measured brightness vectors
    ...
    if cos_m_vals.size > 0 and cos_m_vals.max() > max_m_table:
        raise ValueError(
            f"cos_m_vals contains M > {max_m_table}; extend the Bessel-zeros table first."
        )
    if sin_m_vals.size > 0 and sin_m_vals.max() > max_m_table:
        raise ValueError(
            f"sin_m_vals contains M > {max_m_table}; extend the Bessel-zeros table first."
        )
    ...
    time, brightness1, _, _ = read_xray_brightness(shot, array=1, fix_bad_channels=True)
    print("Reading brightness array 3 ...")
    _, brightness3, _, _ = read_xray_brightness(shot, array=3, fix_bad_channels=True)

    # Combined brightness matrix  shape (npts_bright, ndet_total=76)
    brightness = np.concatenate([brightness1, brightness3], axis=1)
    ndet_total = brightness.shape[1]

    # Optional masking of chords used by inversion
    if chord_mask is None:
        chord_mask_arr = np.ones(ndet_total, dtype=bool)
    else:
        chord_mask_arr = np.asarray(chord_mask, dtype=bool).ravel()
        if chord_mask_arr.size != ndet_total:
            raise ValueError(f"chord_mask has length {chord_mask_arr.size}; expected {ndet_total}.")
    ndet_used = int(chord_mask_arr.sum())
    if ndet_used == 0:
        raise ValueError("chord_mask disabled all chords; inversion cannot proceed.")
    ...
    if read_settings_from_tree:
        ...
        if s.get("cos_m_vals") is not None:
            cos_m_vals = np.asarray(s["cos_m_vals"], dtype=int)
        if s.get("sin_m_vals") is not None:
            sin_m_vals = np.asarray(s["sin_m_vals"], dtype=int)
        if s.get("lmax") is not None:
            lmax = int(s["lmax"])
        if s.get("rmin") is not None:
            rmin = float(s["rmin"])
        if s.get("rmax") is not None:
            rmax = float(s["rmax"])
        if s.get("zmin") is not None:
            zmin = float(s["zmin"])
        if s.get("zmax") is not None:
            zmax = float(s["zmax"])
        if s.get("dr") is not None:
            dr = float(s["dr"])
        if s.get("dz") is not None:
            dz = float(s["dz"])
        if s.get("svd_tol") is not None:
            svd_tol = float(s["svd_tol"])
    ...
    if use_efit_times or use_efit_center or auto_calc_rnorm:
        ...
        efit_times, r_magx, z_magx, rbbbs, zbbbs, nbbbs = read_efit_data(shot, tree=efit_tree)
    ...
    if use_efit_times:
        ...
        if tstart is None or tstop is None or dt is None:
            raise ValueError("Provide tstart, tstop, dt (seconds) or set use_efit_times=True.")
    ...
    if ndet < num_coeffs:
        raise ValueError(
            f"Not enough active chords for inversion: {ndet} < {num_coeffs} coefficients. "
            "Reduce lmax / harmonic count or relax chord masking."
        )
    ...
    for it in range(npts):
        ...
        if redo_matrix:
            ...
            p_full = np.concatenate([p1, p3])
            phi_full = np.concatenate([phi1, phi3])
            p = p_full[chord_mask_arr]
            phi = phi_full[chord_mask_arr]
...
def read_core_emissivity(shot: int):
    ...
    try:
        conn = open_tree(shot, "xtomo")
        emiss = np.asarray(conn.get("\\xtomo::top.tomography:emissivity").data(), dtype=float)
        r = np.asarray(conn.get("dim_of(\\xtomo::top.tomography:emissivity,0)").data(), dtype=float)
        z = np.asarray(conn.get("dim_of(\\xtomo::top.tomography:emissivity,1)").data(), dtype=float)
        t = np.asarray(conn.get("dim_of(\\xtomo::top.tomography:emissivity,2)").data(), dtype=float)
        conn.closeAllTrees()
...
def main() -> None:
    ...
    parser.add_argument(
        "--efit-tree", default="analysis", help="MDSplus tree for EFIT data (default: analysis)"
    )
    parser.add_argument(
        "--no-efit-center",
        action="store_true",
    ...
    parser.add_argument(
        "--no-auto-rnorm", action="store_true", help="Do not auto-compute rnorm from LCFS"
    )
