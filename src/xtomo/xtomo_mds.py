...
def read_xray_brightness(shot: int, array: int = 1, fix_bad_channels: bool = True):
    """
    ...
    try:
        bc_raw = conn.get(f'getnci("\\top.signals.array_{array}:chord_*","state")').data()
...
def bipolar_radii(chord_radii: np.ndarray, chord_angles: np.ndarray, array: int) -> np.ndarray:
    """
    ...
    p = chord_radii.copy()
    neg = (phi_deg > 90.0) & (phi_deg < 270.0) if array == 1 else phi_deg > 180.0
    p[neg] = -p[neg]
    return p
...
def chord_radii_for_array(
    array_idx: int, R_det, Z_det, slopes, rcenter: float, zcenter: float, nchords: int
) -> np.ndarray:
    """
    ...
    p = (
        -sl * rcenter + zcenter + sl * R_det[:nchords, array_idx] - Z_det[:nchords, array_idx]
    ) / np.sqrt(sl**2 + 1.0)
    return np.abs(p)


def chord_angles_for_array(
    array_idx: int, R_det, Z_det, slopes, rcenter: float, zcenter: float, nchords: int
) -> np.ndarray:
    """
...
def read_efit_psi(shot: int, time: float, tree: str = "analysis"):
    ...
    )  # Same result as '\\analysis::top.efit.results.g_eqdsk:psirz'
    ...
    psi_ax = np.asarray(
        conn.get("\\analysis::top.efit.results.a_eqdsk:simagx").data(), dtype=float
    )  # Updated from MDS+ path:'\\efit_aeqdsk:ssimag'
    psi_bndry = np.asarray(
        conn.get("\\analysis::top.efit.results.a_eqdsk:sibdry").data(), dtype=float
...
def read_vessel_tiles(shot: int, tree: str = "analysis"):
    ...
    def _read_segs(name):
        ...
        prefix = (
            "tile" if name == "tiles" else name
        )  # 'tile' or 'vessel' [May have been 'vess' at some point?]
...
def read_tomography_settings(shot: int) -> dict:
    ...
    for key, node in [
        ("center_times", "dim_of(\\top.tomography.input_params:r_center)"),
