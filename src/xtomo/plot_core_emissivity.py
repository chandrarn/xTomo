...
def plot_core_emissivity(
    ...
    *,
    ...
    rmin_ax = 0.40; rmax_ax = 0.95
    zmin_ax = -0.50; zmax_ax =  0.50
    ...
    if ax is None:
        ...
        fig.canvas.manager.set_window_title(
            f'X-ray tomography  shot {shot}') if hasattr(
    ...
    if not nogrid:
        ...
        ax.set_title(
            f'X-ray tomography  shot {shot},  t = {t_plot:.3f} s',
            fontsize=12)
    ...
    emiss_slice = emissivity[:, :, t_idx]          # (nr, nz)
    ...
    pcm = ax.pcolormesh(
        Redge, Zedge, emiss_slice,
        cmap=cmap, vmin=vmin, vmax=vmax, shading='flat',
        zorder=1,
    )
    fig.colorbar(pcm, ax=ax, label='Emissivity (W/m³)', shrink=0.8)

    # ---- EFIT flux contours ----------------------------------------------
    if not noflux:
        try:
            psi, R_efit, Z_efit, psi_ax, psi_bndry = read_efit_psi(
                shot, t_plot, tree=efit_tree)
            ...
            levels_inner = (np.arange(n_flux_contours + 1) / (n_flux_contours + 1)
                            if n_flux_contours > 0 else [])
    ...
    if not novessel:
        try:
            ...
            for R_seg, Z_seg in tiles:
                ax.plot(R_seg, Z_seg, color='white', linewidth=0.8,
                        alpha=0.9, zorder=3)
            for R_seg, Z_seg in vessel:
                ax.plot(R_seg, Z_seg, color='cyan', linewidth=0.8,
                        alpha=0.7, zorder=3)
        except Exception as exc:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description='Plot 2-D XTOMO emissivity for a C-Mod shot')
    ...
    emissivity, r, z, t, ok = core_xray_emissivity(
        args.shot,
        tstart=tstart, tstop=tstop, dt=args.dt,
